...
Matrix* transpose(Matrix* A){
    Matrix* A_T = malloc(sizeof(Matrix));
    int i,j;
    /*Initializing the matrix*/
    A_T->n_rows = A->n_cols;
    A_T->n_cols = A->n_rows;
...
Matrix* symnmf(Matrix* H, Matrix* W){
    ...
    int iter = 0,i,j;
    Matrix* cur_H = malloc(sizeof(Matrix));/*Initializing matrices*/
    Matrix* WH = malloc(sizeof(Matrix));
    Matrix* HT = malloc(sizeof(Matrix));
    Matrix* HHTH = malloc(sizeof(Matrix));
    Matrix* HHT = malloc(sizeof(Matrix));
    Matrix* last_H = H;
    ...
    free(WH),free(HT),free(HHTH),free(HHT);
    return cur_H;
}
